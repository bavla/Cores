...
class Search(object):
    ...
    def pathTo(self,t):
        ...
        u = t; path = []; s = self.getGraph('source')
        while u != s:
    ...
    def BFSpaths(self,s):
        ...
        self.BFS(s)
    def BFS(self,v):
        self.setNode(v,'mark',True); s = self.getGraph('source')
        q = [s]
        while len(q) > 0:
            v = q.pop(0)
            for w in self.outNeighbors(v):
    ...
    def connectedComponents(self):
        ...
        for w in self.nodes():
            if self.getNode(w,'weak') == 0:
                nc += 1; self.setGraph('weak',nc); self.DFScc(w)
    def DFScc(self,v):
        self.setNode(v,'weak',self.getGraph('weak'))
        for w in self.neighbors(v):
            if self.getNode(w,'weak') == 0: self.DFScc(w)
    def weaklyConnected(self,u,v):
        return self.getNode(u,'weak') == self.getNode(v,'weak')
    def weakCompNum(self): return self.getGraph('weak')

    def searchBipart(self):
        self.setGraph('bipart',True); self.setNodes('mark',False)
    ...
    def DFSbi(self,v):
        ...
        for w in self.neighbors(v):
            if not self.getNode(w,'mark'):
                ...
                self.setGraph('bipart',False)
    def isBipartite(self): return self.getGraph('bipart')

    def searchCycle(self):
        self.setGraph('cyclic',False); self.setNodes('mark',False)
        for w in self.nodes():
    ...
    def DFScyc(self,v,u):
        ...
        for w in self.neighbors(v):
            ...
            elif w != u: self.setGraph('cyclic',True)
    def isCyclic(self): return self.getGraph('cyclic')
    ...
    def KosarajuSCC(self):
        ...
        for w in order:
            if self.getNode(w,'strong') == 0:
                nc += 1; self.setGraph('strong',nc); self.DFSscc(w)
    def DFSscc(self,v):
        self.setNode(v,'strong',self.getGraph('strong'))
        for w in self.outNeighbors(v):
            if self.getNode(w,'strong') == 0: self.DFSscc(w)
    def stronglyConnected(self,u,v):
        return self.getNode(u,'strong') == self.getNode(v,'strong')
    def strongCompNum(self): return self.getGraph('strong')
...
#  Assumes all weights are nonnegative.
    ...
    def DijkstraSP(self, s):
        self.setGraph('source',s)   # source node
